                ...
                if(window.global_user){
                    ...
                    if(!isDomain() || getUrlParam('nowx')){
                       // global_user = {openid:'10087', nickname:'viola'};
                        global_user = {openid:'o2XYKv6SbhrZa-RjG2V2JO9dsKLU', nickname:'xuli'};
                        callback && callback();
                        return;
                    }

                    if (document.domain == 'dev.hrtell.com') {
                        this.get(getAPIHOST() + '/app/wx/info?url=' + encodeURIComponent(location.href), function (res) {
                            var conf_data = res;
                            var state = 'http://dev.hrtell.com/'+getCorporation()+'/app/wx/proxy-js';
                            var redirectUrl = encodeURIComponent('http://www.hrtell.com/xuni/app/wx/proxy');
